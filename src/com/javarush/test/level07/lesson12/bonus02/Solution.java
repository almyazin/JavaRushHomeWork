...
public class Solution
{
    public static void main(String[] args) throws IOException
    {
        ...
        ArrayList<String> listExtended = new ArrayList<>();
        for (int i = 0; i < list.size(); i++)
        {
            String s = list.get(i);
            listExtended.add(s.length() % 2 == 0 ? s + " " + s : s + " " + s + " " + s);
        }

        for (int i = 0; i < listExtended.size(); i++)
        {
            System.out.println(listExtended.get(i));
        }
    }
}
